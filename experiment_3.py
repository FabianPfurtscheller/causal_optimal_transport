...
### pi = (mu, nu), mu_1 ~ N(0,1), mu_2 = mu_1, nu_1 ~ N(0,4), nu_2 = nu_1

def sample_pi(batch_size):
    while 1:
        mu = np.random.normal(0, 1, [batch_size])
        nu = np.random.normal(0, 4, [batch_size])
        pi = np.stack((np.array([mu,mu]).T,np.array([nu,nu]).T), axis = 1)
        yield pi
